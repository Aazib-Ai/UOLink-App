...
export function AuraDashboardContent({
  ...
  const auraInfo = getAuraTier(auraStats.currentAura)
  ...
  const growthHighlights = useMemo(
    ...
    [auraStats.recentActivity.auraGainedThisWeek, auraStats.recentActivity.notesThisWeek, auraStats.topNote]
  )
  ...
  const cautions = useMemo(
    ...
    [auraStats.recentActivity.notesThisWeek]
  )
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-lime-100 bg-white/95 p-6 shadow-sm">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-2xl border border-lime-100 bg-white px-4 py-4 shadow-sm">
            ...
            <p className="mt-3 text-2xl font-semibold text-[#1f2f10]">{auraStats.totalNotes}</p>
          ...
          <div className="rounded-2xl border border-blue-100 bg-white px-4 py-4 shadow-sm">
            ...
            <p className="mt-3 text-2xl font-semibold text-blue-900">{auraStats.totalUpvotes}</p>
            <p className="mt-1 text-xs text-blue-800/70">Showing community approval</p>
          </div>
          <div className="rounded-2xl border border-rose-100 bg-white px-4 py-4 shadow-sm">
            ...
            <p className="mt-3 text-2xl font-semibold text-rose-900">{auraStats.totalSaves}</p>
            <p className="mt-1 text-xs text-rose-700/70">Classmates keeping your notes</p>
          </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          {growthHighlights.map(({ icon: Icon, title, value, hint }) => (
            <div key={title} className="flex items-start gap-3 rounded-2xl border border-lime-100 bg-[#f7fbe9]/60 px-4 py-4">
              <Icon className="mt-0.5 h-9 w-9 rounded-full bg-white p-2 text-[#90c639] shadow-sm" />
              <div>
                <p className="text-sm font-semibold text-[#1f2f10]">{title}</p>
                <p className="mt-1 text-lg font-semibold text-[#334125]">{value}</p>
                <p className="mt-1 text-xs text-[#5f7050]">{hint}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border border-lime-100 bg-white/95 p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-[#1f2f10]">
              <Target className="h-5 w-5 text-[#90c639]" />
              Build your aura plan
            </div>
            <div className="space-y-4">
              {playbook.map(({ icon: Icon, step, description }, index) => (
                <div key={step} className="flex items-start gap-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#f7fbe9] text-sm font-semibold text-[#90c639]">
                    {index + 1}
                  </div>
                  <div>
                    <p className="font-medium text-[#1f2f10]">{step}</p>
                    <p className="text-sm text-[#4c5c3c]">{description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4 text-sm text-blue-900/80">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p>Drop links in study chats or discord servers to spark saves.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p>Updated notes rise back to the top and earn fresh reactions.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p>Highlight your aura badge on your profile to build trust instantly.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
            ...
            <div className="space-y-4 text-sm text-amber-900/90">
        ...
        <section className="rounded-3xl border border-lime-100 bg-white/95 p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-lg font-semibold text-[#1f2f10]">
              <Trophy className="h-5 w-5 text-[#90c639]" />
              Aura tiers
            </div>
            <button
      ...
      {showFullFeatures && !auraInfo.isMaxTier && (
        <section className="rounded-3xl border border-orange-200 bg-gradient-to-br from-orange-50 via-yellow-50 to-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h3 className="mt-2 text-2xl font-semibold text-orange-900">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <div className="rounded-full border border-orange-200 bg-white px-5 py-3 text-center text-sm font-medium text-orange-800 shadow-sm">
                {auraStats.totalNotes} notes shared • {auraStats.totalSaves} saves • {auraStats.totalUpvotes} upvotes
              </div>
            </div>
          </div>
        </section>
